...
def load_trees(dirpath):
    ...
    with open(os.path.join(dirpath, "parents.txt")) as parentsfile, open(
        os.path.join(dirpath, "dparents.txt")
    ) as dparentsfile, open(os.path.join(dirpath, "sents.txt")) as toksfile:
        parents, dparents = [], []
        ...
        for i in range(len(toks)):
            const_trees.append(load_constituency_tree(list(parents[i]), toks[i]))
            dep_trees.append(load_dependency_tree(list(dparents[i])))
...
def split(sst_dir, train_dir, dev_dir, test_dir):
    ...
    with open(os.path.join(train_dir, "sents.txt"), "w") as train, open(
        os.path.join(dev_dir, "sents.txt"), "w"
    ) as dev, open(os.path.join(test_dir, "sents.txt"), "w") as test, open(
        os.path.join(train_dir, "parents.txt"), "w"
    ) as trainparents, open(
        os.path.join(dev_dir, "parents.txt"), "w"
    ) as devparents, open(
        os.path.join(test_dir, "parents.txt"), "w"
    ) as testparents:
...
def write_labels(dirpath, dictionary):
    ...
    with open(os.path.join(dirpath, "labels.txt"), "w") as labels, open(
        os.path.join(dirpath, "dlabels.txt"), "w"
    ) as dlabels:
